...
const ccpigdFreeDownloadButtons = document.querySelectorAll(".ccpigd-download-btn");

if (ccpigdFreeDownloadButtons.length > 0) {

    // DOWNLOAD FILE + CLOSE POPUP ON SUCCESS
    document.addEventListener('wpcf7mailsent', function(event) {
        const ccpigdFreeDownloadFileUrl = "https://downloads.wordpress.org/plugin/integration-google-drive.zip";
        const ccpigdFreeDownloadForm = event.target;

        if (ccpigdFreeDownloadForm.closest('.free-downolad-form')) {

            // Trigger file download
            const ccpigdFreeDownloadAnchor = document.createElement('a');
            ccpigdFreeDownloadAnchor.href = ccpigdFreeDownloadFileUrl;
            ccpigdFreeDownloadAnchor.download = "";
            document.body.appendChild(ccpigdFreeDownloadAnchor);
            ccpigdFreeDownloadAnchor.click();
            document.body.removeChild(ccpigdFreeDownloadAnchor);

            // Close popup after successful submit
            const ccpigdFreeDownloadPopupSection = document.querySelector(".ccpigd-download-popup-section");
            if (ccpigdFreeDownloadPopupSection) ccpigdFreeDownloadPopupSection.classList.remove("active");
        }
    });
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const ccpigdFreeDownloadPopupSection = document.querySelector(".ccpigd-download-popup-section");
        const ccpigdFreeDownloadPopupBox = document.querySelector(".ccpigd-download-popup"); 
        const ccpigdFreeDownloadCloseBtns = document.querySelectorAll(".ccpigd-popup-close-btn");

        if (!ccpigdFreeDownloadPopupSection || !ccpigdFreeDownloadPopupBox) return;

        // OPEN POPUP ON BUTTON CLICK
        ccpigdFreeDownloadButtons.forEach(btn => {
            btn.addEventListener("click", function() {
                ccpigdFreeDownloadPopupSection.classList.add("active");
            });
        });
        ...
        document.addEventListener("click", function(e) {
            if (
                ccpigdFreeDownloadPopupSection.classList.contains("active") &&
                !ccpigdFreeDownloadPopupBox.contains(e.target) &&
                !e.target.closest(".ccpigd-download-btn")
            ) {
                ccpigdFreeDownloadPopupSection.classList.remove("active");
            }
        });

        // CLOSE POPUP ON ANY CLOSE BUTTON CLICK
        if (ccpigdFreeDownloadCloseBtns.length > 0) {
            ccpigdFreeDownloadCloseBtns.forEach(btn => {
                btn.addEventListener("click", function() {
                    ccpigdFreeDownloadPopupSection.classList.remove("active");
                });
            });
